...
if [ ! -f $dict_dir/cmudict/cmudict.0.7a ]; then
  ...
  svn co -r 13068 http://svn.code.sf.net/p/cmusphinx/code/trunk/cmudict \
...
awk 'NR==FNR{words[tolower($1)]; next;} !(tolower($1) in words)' \
  $dict_dir/cmudict/cmudict-plain.txt $dict_dir/lexicon-en/words-en.txt |\
  egrep -v '<.?s>' | awk '{print toupper($0)}' > $dict_dir/lexicon-en/words-en-oov.txt || exit 1;

awk 'NR==FNR{words[tolower($1)]; next;} (tolower($1) in words)' \
  $dict_dir/lexicon-en/words-en.txt $dict_dir/cmudict/cmudict-plain.txt |\
  egrep -v '<.?s>' | awk '{print tolower($0)}' > $dict_dir/lexicon-en/lexicon-en-iv.txt || exit 1;
...
g2p.py --model=conf/g2p_model --apply $dict_dir/lexicon-en/words-en-oov.txt | awk '{print tolower($0)}' \
...
cat conf/cmu2pinyin |awk '{print tolower($0)}'| awk '{print $1;}' | sort -u > $dict_dir/map/cmu || exit 1;
cat conf/pinyin2cmu | awk '{print tolower($0)}'| awk -v cmu=$dict_dir/map/cmu \
...
awk 'NR==FNR{words[tolower($1)]; next;} (tolower($1) in words)' \
  ...
  egrep -v '<.?s>'| awk '{print tolower($0)}' > $dict_dir/map/cmu-py || exit 1;
...
' conf/pinyin2cmu |awk '{print tolower($0)}' > $dict_dir/map/cmu-cmu || exit 1;
...
' $dict_dir/map/cmu-cmu | awk '{print tolower($0)}' > $dict_dir/lexicon-en/lexicon-en.txt || exit 1;
...
if [ ! -f $dict_dir/cedict/cedict_1_0_ts_utf-8_mdbg.txt ]; then
  ...
  wget --no-check-certificate -P $dict_dir/cedict https://www.mdbg.net/chinese/export/cedict/cedict_1_0_ts_utf-8_mdbg.txt.gz
